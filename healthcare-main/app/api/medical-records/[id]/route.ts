...
import { MedicalRecord, RecordType, DeletedAttachment } from "@/types/medical-records";
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch medical record" } as any, { status: 500 });
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    if (deletedAttachments.length > 0) {
      const attachmentsToDelete = existingRecord.attachments.filter((att) =>
        deletedAttachments.some((deleted) => deleted.id === att.id),
      );
    ...
    return NextResponse.json({ error: "Failed to update medical record" } as any, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to delete medical record" } as any, { status: 500 });
  }
}
