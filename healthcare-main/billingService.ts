...
class BillingService {
  ...
  public async markTimeEntriesAsInvoiced(timeEntryIds: string[], invoiceId: string): Promise<void> {
    try {
  ...
  public async getInvoicesWithTimeEntries(clientId?: string, status?: string): Promise<Invoice[]> {
    try {
      // Build query parameters
      const params = new URLSearchParams();
      ...
      Object.entries(grouped).forEach(([groupId, entries]) => {
        ...
        if (totalHours > 0) {
          ...
          lineItems.push({
            id: "", // Will be assigned by the server
            description:
              groupBy === "project" ? `Project work: ${groupId}` : `Task work: ${groupId}`,
            quantity: totalHours,
            rate: avgRate,
            amount: totalAmount,
