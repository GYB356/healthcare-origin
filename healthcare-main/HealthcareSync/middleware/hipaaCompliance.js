  ...
  try {
    ...
    if (containsPHI(req.path)) {
      ...
      if (!user || !user.hipaaConsent || !user.hipaaConsent.status === "accepted") {
        logger.warn(
          `HIPAA consent not accepted: User ${req.user.id} attempted to access ${req.path}`,
        );
        return res.status(403).json({
          error: "HIPAA consent required",
          message:
            "You must accept the HIPAA consent agreement before accessing protected health information.",
