...
const VitalSignsDashboard = () => {
        ...
        [data.patientId]: {
          ...
          [data.type]: {
            ...
            trend: prev[data.patientId]?.[data.type]?.value
              ? data.value > prev[data.patientId][data.type].value
                ? "increasing"
                : "decreasing"
              : "stable",
          },
  ...
  const getVitalSignStatus = (type, value) => {
    ...
    if (value < range.critical_min || value > range.critical_max) {
      return {
        status: value < range.critical_min ? "critical-low" : "critical-high",
        severity: "critical",
      };
    }
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {alerts.map((alert, index) => (
            <div
              key={index}
              className={`border-l-4 p-4 ${alert.priority === "high" ? "bg-red-100 border-red-500" : "bg-yellow-100 border-yellow-500"}`}
            >
              ...
              {Object.entries(signs).map(([type, data]) => {
                ...
                return (
                  ...
                  >
                    ...
                    {severity !== "normal" && (
                      <p
                        className={`text-sm ${severity === "critical" ? "text-red-600" : "text-yellow-600"}`}
                      >
