...
const ProtectedRoute = ({
  ...
  if (requiredRole) {
    ...
    if (!hasRequiredRole) {
      ...
      console.log(
        `HIPAA Log: Unauthorized role access attempt by ${user.id} (${user.role}) to ${location.pathname}`,
      );
  ...
  if (requiredPermission) {
    ...
    if (!hasRequiredPermission) {
      // Log unauthorized access attempt
      console.log(
        `HIPAA Log: Unauthorized permission access attempt by ${user.id} (${user.role}) to ${location.pathname}`,
      );
      return <Navigate to="/unauthorized" replace />;
    }
  }
