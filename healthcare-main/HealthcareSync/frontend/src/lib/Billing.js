...
const Billing = () => {
  ...
  const handleEditInsurance = async (insuranceId) => {
    try {
      ...
      if (response.ok) {
        setInsuranceDetails((prev) => prev.map((ins) => (ins.id === insuranceId ? data : ins)));
        setShowInsuranceModal(false);
        setInsuranceForm({
          provider: "",
          policyNumber: "",
  ...
  const handleDeleteInsurance = async (insuranceId) => {
    ...
    try {
      ...
      if (response.ok) {
        setInsuranceDetails((prev) => prev.filter((ins) => ins.id !== insuranceId));
      } else {
        const data = await response.json();
  ...
  useEffect(() => {
    ...
    socket.on("insuranceDeleted", (deletedInsuranceId) => {
      setInsuranceDetails((prev) => prev.filter((ins) => ins.id !== deletedInsuranceId));
    });
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6" aria-label="Billing Management">
        Billing Management
      </h1>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-start">
                <h2 className="text-xl font-bold mb-4" aria-label="Invoice Details">
                  Invoice Details
                </h2>
                <button
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-start">
                <h2 className="text-xl font-bold mb-4" aria-label="Create New Invoice">
                  Create New Invoice
                </h2>
                <button
            ...
            <div className="p-6">
              <div className="flex justify-between items-start">
                <h2
                  ...
                  aria-label="{selectedPaymentMethod ? 'Edit Payment Method' : 'Add Payment Method'}"
                >
              ...
              <form
                onSubmit={
                  selectedPaymentMethod
                    ? () => handleEditPaymentMethod(selectedPaymentMethod.id)
      ...
      <div className="mt-8">
        <h2 className="text-xl font-bold mb-4" aria-label="Insurance Information">
