    ...
    const fetchAnalyticsData = async () => {
      try {
        ...
        const patientResponse = await fetch(
          `/api/analytics/patients?timeRange=${timeRange}&department=${department}`,
        );
        const financialResponse = await fetch(
          `/api/analytics/financial?timeRange=${timeRange}&department=${department}`,
        );
        const operationalResponse = await fetch(
          `/api/analytics/operational?timeRange=${timeRange}&department=${department}`,
        );
        const clinicalResponse = await fetch(
          `/api/analytics/clinical?timeRange=${timeRange}&department=${department}`,
        );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <p className="text-3xl font-bold text-green-600">
            ${(data.financialMetrics.totalRevenue / 1000).toFixed(1)}k
          </p>
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <p className="text-3xl font-bold text-yellow-600">
            {data.operationalMetrics.bedOccupancy}%
          </p>
          <div className="mt-2 text-sm">
            <div className="w-full bg-gray-200 rounded-full h-2.5">
              <div
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <p className="text-3xl font-bold text-purple-600">
            {data.clinicalMetrics.patientSatisfaction}%
          </p>
          <div className="mt-2 text-sm">
            <div className="w-full bg-gray-200 rounded-full h-2.5">
              <div
                className="bg-purple-600 h-2.5 rounded-full"
                style={{ width: `${data.clinicalMetrics.patientSatisfaction}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="flex items-center justify-center h-64">
            <div className="w-48 h-48 rounded-full border-8 border-gray-200 relative">
              {Object.entries(data.clinicalMetrics.treatmentOutcomes).map(
                ([key, value], index, arr) => {
                  const total = Object.values(data.clinicalMetrics.treatmentOutcomes).reduce(
                    (a, b) => a + b,
                    0,
                  );
                  ...
                  const prevPercentages = arr
                    .slice(0, index)
                    .reduce((sum, [_, val]) => sum + (val / total) * 100, 0);
