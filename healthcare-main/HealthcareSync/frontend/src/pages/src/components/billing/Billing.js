...
const Billing = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (patientId) {
        const paymentMethodsResponse = await fetchWithSecurityHeaders(
          `/api/billing/payment-methods?patientId=${patientId}`,
        );
        ...
        const insuranceResponse = await fetchWithSecurityHeaders(
          `/api/insurance?patientId=${patientId}`,
        );
  ...
  const handlePaymentMethodDelete = (deletedId) => {
    setPaymentMethods((prevMethods) => prevMethods.filter((method) => method._id !== deletedId));
  };
  ...
  const handleInsuranceDelete = (deletedId) => {
    setInsuranceData((prevData) => prevData.filter((data) => data._id !== deletedId));
  };
