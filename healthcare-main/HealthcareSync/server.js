...
app.use(
  "/api/analytics",
  requireAuth,
  requireRole(["admin", "doctor", "manager"]),
  analyticsRoutes,
);
...
io.on("connection", (socket) => {
  ...
  socket.on("device_data", async (data) => {
    try {
      ...
      logger.info(
        `Device data received from user ${userId}: ${data.type} - ${data.value}${data.unit}`,
      );
    } catch (err) {
  ...
  socket.on("consultation_update", async (data) => {
    try {
      ...
      const updateResult = await TelemedicineService.updateConsultation(
        data.consultationId,
        data.update,
      );
      if (updateResult) {
