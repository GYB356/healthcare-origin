...
router.post("/", authenticateJWT, async (req, res) => {
  try {
    ...
    if (req.user.id !== validatedData.senderId && req.user.role !== "ADMIN") {
      return res
        .status(403)
        .json({ error: "Unauthorized attempt to send message as another user" });
    }
...
router.delete("/:messageId", authenticateJWT, async (req, res) => {
  try {
    ...
    if (
      ...
      req.user.id !== message.recipientId &&
      req.user.role !== "ADMIN"
    ) {
    ...
    const updateData =
      req.user.id === message.senderId ? { senderDeleted: true } : { recipientDeleted: true };
