...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: error instanceof Error ? error.message : "Failed to create subscription" });
  }
});
...
router.post("/cancel", requireAuth, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: error instanceof Error ? error.message : "Failed to cancel subscription" });
  }
});
