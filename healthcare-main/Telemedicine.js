...
function Telemedicine() {
  ...
  const fetchConsultations = useCallback(async (id, role) => {
    try {
      setIsLoading(true);
      const endpoint =
        role === "patient" ? `/api/consultations/patient/${id}` : `/api/consultations/doctor/${id}`;
  ...
  const filterConsultations = () => {
    ...
    if (activeTab === "upcoming") {
      return consultations.filter(
        (c) => new Date(c.scheduledTime) > now && c.status !== "completed",
      );
  ...
  return (
    <div className="container mx-auto px-4 py-8" role="main">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-primary" tabIndex="0">
...
function ConsultationItem({ consultation, userRole, onJoin }) {
  ...
  const displayName =
    userRole === "patient" ? `Dr. ${consultation.doctorName}` : consultation.patientName;
    ...
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      <div className="bg-white shadow-sm p-4">
        <div className="container mx-auto flex justify-center gap-4">
          ...
          <button onClick={endCall} className="rounded-full p-3 bg-red-500 text-white">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
