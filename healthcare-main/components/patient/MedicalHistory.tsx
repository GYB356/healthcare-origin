...
import { DocumentTextIcon, CalendarIcon, ClockIcon, UserIcon } from "@heroicons/react/24/outline";
          ...
          {records.map((record) => (
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-lg font-medium text-gray-900">{record.condition}</h3>
                  <p className="mt-1 text-sm text-gray-500">{record.diagnosis}</p>
                </div>
                <span
                  className={`px-2 py-1 text-xs font-semibold rounded-full ${
                    record.endDate ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                  }`}
                >
