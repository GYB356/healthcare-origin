...
export class A11yAppScanner {
  ...
  private async scanRoute(route: RouteDefinition, parentPath: string = ""): Promise<void> {
    ...
    if (this.scanOptions.includeNestedRoutes && route.childRoutes && route.childRoutes.length > 0) {
  ...
  private async auditRoute(
    ...
    const componentName =
      options.customName || route.component || `Route: ${route.name || route.path}`;

    try {
      // Run the audit
      const auditResult = await a11yAuditService["runA11yAudit"](auditTarget, {});
  ...
  public getScanProgress(): { completed: number; total: number; percentage: number } {
    const percentage =
      this.totalRoutes > 0 ? Math.round((this.completedRoutes / this.totalRoutes) * 100) : 0;
