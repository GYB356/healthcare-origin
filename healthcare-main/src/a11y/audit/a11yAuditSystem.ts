...
export class A11yAuditSystem {
  ...
  private logViolationsSummary(route: string, results: AxeResults): void {
    ...
    violations.forEach((violation) => {
      ...
      console.log(
        chalk.yellow(
          `- ${violation.id} (${impact}): ${violation.help} (${violation.nodes.length} occurrences)`,
        ),
      );
    });
  ...
  private createSummaryReport(reportDir: string, violationData: ViolationData): void {
    const { totalViolations, routeViolations, impactCounts, mostCommonViolations } = violationData;
    ...
    fs.writeFileSync(path.join(reportDir, "summary.md"), summaryContent.join("\n"));
  }
  ...
  private createRouteReport(reportDir: string, result: AuditResult): void {
    ...
    if (result.html) {
      fs.writeFileSync(path.join(routeDir, "page.html"), result.html);
    }
    ...
    fs.writeFileSync(path.join(routeDir, "report.md"), routeContent.join("\n"));
  ...
  private createRouteHtmlReport(routeDir: string, result: AuditResult, violations: Result[]): void {
    ...
    fs.writeFileSync(path.join(routeDir, "report.html"), htmlContent);
  }
  ...
  private createHtmlReportIndex(reportDir: string, violationData: ViolationData): void {
    ...
    const sortedRoutes = Object.entries(routeViolations).sort(
            ...
            <div class="common-violations">
              ${mostCommonViolations
                ...
                <div class="violation-item">
                  ...
                  <h4>Common fixes:</h4>
                  <ul>
                    ${this.generateFixSuggestions(violation.id, { html: "", failureSummary: "" })
                      .map(
                        (suggestion) => `
                      <li>${suggestion}</li>
                    `,
  ...
  public static async generateDiffReport(
    ...
    try {
      ...
      // Save diff data
      fs.writeFileSync(path.join(outputDir, "diff-data.json"), JSON.stringify(diffData, null, 2));
      ...
      fs.writeFileSync(path.join(outputDir, "diff-report.html"), diffHtml);
  ...
  public getBestPractices(): string[] {
    ...
    if (mostCommonViolations.length > 0) {
      bestPractices.push("## Specific Recommendations", "");
      ...
      mostCommonViolations.forEach((violation) => {
        ...
        switch (violation.id) {
          case "color-contrast":
            recommendation =
              "Ensure sufficient color contrast (4.5:1 for normal text, 3:1 for large text) between text and background colors.";
            break;

          case "image-alt":
            recommendation =
              'Add descriptive alt text to all images. Use empty alt attributes (alt="") for decorative images.';
            break;

          case "label":
            recommendation =
              "Associate labels with form controls using the for/id attributes or by nesting inputs inside label elements.";
            break;

          case "document-title":
            recommendation =
              "Provide a descriptive page title that identifies the content of the page.";
            break;

          case "aria-roles":
            recommendation =
              "Use ARIA roles correctly according to the ARIA specification. Consider using semantic HTML elements instead when possible.";
            break;
          ...
          case "heading-order":
            recommendation =
              "Maintain a logical heading structure with no skipped levels (e.g., h1 to h3 without h2).";
            break;

          case "link-name":
            recommendation =
              "Ensure links have discernible text that clearly indicates their purpose.";
            break;

          case "button-name":
            recommendation =
              "Provide text content or aria-label for buttons that describes their function.";
            break;
