  ...
  useEffect(() => {
    const fetchEmergencyData = async () => {
      try {
        ...
        setSelectedContacts(
          contacts.filter((contact) => contact.isAuthorized).map((contact) => contact.id),
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedContacts.length > 0) {
        await EmergencyService.sendEmergencyNotifications(createdAlert.id, selectedContacts);
      }
      ...
      setError(
        "Failed to send emergency alert. Please call emergency services directly if this is a life-threatening situation.",
      );
    } finally {
      setSending(false);
    }
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div className="flex">
              <button
                ...
                className={`flex-1 py-2 ${
                  severity === "moderate" ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700"
                } rounded-l-md`}
              ...
              <button
                type="button"
                className={`flex-1 py-2 ${
                  severity === "urgent" ? "bg-yellow-500 text-white" : "bg-gray-100 text-gray-700"
                }`}
              ...
              <button
                type="button"
                className={`flex-1 py-2 ${
                  severity === "critical" ? "bg-red-500 text-white" : "bg-gray-100 text-gray-700"
                } rounded-r-md`}
      ...
      <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
        <p className="text-xs text-gray-500">
          Emergency alerts are monitored 24/7 by our healthcare team. Response times may vary
          depending on alert severity and staff availability.
        </p>
      </div>
    </div>
  );
