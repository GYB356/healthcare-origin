...
const BAACompare = ({ baaId, versions, onClose }) => {
  ...
  const fetchVersionContents = async () => {
    try {
      ...
      const leftTerms =
        typeof leftData.terms === "string" ? decryptData(leftData.terms) : leftData.terms;

      const rightTerms =
        typeof rightData.terms === "string" ? decryptData(rightData.terms) : rightData.terms;
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
        <h3 className="text-lg leading-6 font-medium text-gray-900">Compare BAA Versions</h3>
                            ...
                            <li key={i} className="text-xs">
                              ...
                              {change.type === "changed" && (
                                <span className="text-amber-600">
                                  Line {change.lineNumber + 1} changed from "{change.leftContent}"
                                  to "{change.rightContent}"
                                </span>
                              )}
                            </li>
                          ))}
