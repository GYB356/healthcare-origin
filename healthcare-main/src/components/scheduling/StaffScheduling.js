  ...
  return (
    <div className="scheduling-container bg-white rounded-lg shadow p-4">
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        ...
        {conflicts.length > 0 && (
          <ConflictResolutionModal conflicts={conflicts} onResolve={handleResolveConflict} />
        )}
  ...
  showTimeOffModal && (
    <TimeOffRequestModal
      ...
      onSubmit={(newRequest) => {
        ...
        setShifts(shifts.filter((shift) => !checkConflict(shift, [newRequest]).hasConflict));
      }}
    />
  );
}
