...
const PRIMARY_ENCRYPTION_KEY =
  process.env.REACT_APP_PRIMARY_ENCRYPTION_KEY || "default-key-for-development";
...
export const validateHIPAACompliance = (data) => {
  ...
  const failures = requirements.filter((req) => !req.check()).map((req) => req.message);
...
export const sanitizeForLogging = (data) => {
  const sensitiveFields = ["ssn", "creditCard", "password", "dateOfBirth", "phoneNumber", "email"];
  ...
  try {
    ...
    const report = {
      ...
      compliance: {
        ...
        violationsByType: logs
          .filter((log) => log.status === "violation")
          .reduce((acc, log) => {
      ...
      recommendations: [],
      detailedLogs: options.includeDetailedLogs
        ? logs.map((log) => sanitizeForLogging(log))
        : undefined,
    };
    ...
    if (report.compliance.violations.length > 0) {
      report.recommendations.push(
        "Review access control policies and implement additional safeguards",
      );
    }

    if (report.summary.emergencyAccesses > 0) {
      report.recommendations.push(
        "Audit emergency access patterns and verify all emergency access was legitimate",
      );
    }
...
const calculateComplianceScore = (logs, controlType) => {
  ...
  const violations = logs.filter(
    (log) => log.status === "violation" && log.details && log.details.controlType === controlType,
  ).length;

  const total =
    logs.filter((log) => log.details && log.details.controlType === controlType).length || 1; // Avoid division by zero
...
const calculateRiskScore = (logs) => {
  ...
  const highSeverityViolations = logs.filter(
    (log) => log.status === "violation" && log.details && log.details.severity === "high",
  ).length;
...
const detectSuspiciousPatterns = (logs) => {
  ...
  Object.entries(userLogs).forEach(([userId, userLogEntries]) => {
    ...
    const patientAccesses = userLogEntries.filter(
      (log) => log.type === "PATIENT_RECORD_ACCESS" && log.details && log.details.patientId,
    );
    ...
    const timeSpan =
      patientAccesses.length > 0
        ? (new Date(patientAccesses[patientAccesses.length - 1].timestamp) -
            new Date(patientAccesses[0].timestamp)) /
          (1000 * 60)
        : 0; // in minutes

    if (uniquePatients > 20 && timeSpan < 60) {
      // More than 20 patients in less than an hour
      suspiciousPatterns.push({
